...
pub fn main() !void {
    ...
    while (lines.next()) |line| : (row += 1) {
        ...
        for (line, 0..) |char, col| {
            ...
            if (char == 'X') try seeds.append(.{ row, col });
        }
    }
    ...
    for (seeds.items) |seed| {
        //print("X at ({},{})\n", .{ seed.@"0", seed.@"1" });
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", 0, 1);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", 1, 1);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", 1, 0);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", 1, -1);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", 0, -1);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", -1, -1);
        xmas_counter += exploreForPatternInDirection(seed, char_matrix, "XMAS", -1, 0);
