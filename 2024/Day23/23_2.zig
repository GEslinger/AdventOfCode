...
pub fn main() !void {
    ...
    var p: VertRef = .empty;
    var x: VertRef = .empty;
    var max_clique_indexes: VertRef = .empty;
    for (seen_comps.items, 0..) |_, i| {
        try p.append(alloc, i);
    }

    _ = try bk(&r, &p, &x, &max_clique_indexes, &seen_comps, alloc);

    var longest_clique: std.ArrayList([2]u8) = .empty;
    for (max_clique_indexes.items) |idx| {
        try longest_clique.append(alloc, seen_comps.items[idx]);
    }
...
const VertRef = std.ArrayList(usize);
...
fn bk(
    ...
    x: *VertRef,
    max: *VertRef,
    verts: *std.ArrayList([2]u8),
    alloc: std.mem.Allocator,
) !?VertRef {
    if (p.items.len == 0 and x.items.len == 0) return r.*;
    for (0..p.items.len) |_| {
        const p_vert = p.items[0];

        // Get intersection of P, X with neighbors
        var p_next: VertRef = .empty;
        var x_next: VertRef = .empty;
        for (adjacency[p_vert], 0..) |potential_edge, i| {
            // Neighbor condition
            if (potential_edge == 1) {
                for (p.items) |p_n| if (p_n == i) try p_next.append(alloc, i);
                for (x.items) |x_n| if (x_n == i) try x_next.append(alloc, i);
            }
        }

        var r_next = try r.clone(alloc);
        try r_next.append(alloc, p_vert);
        if (try bk(&r_next, &p_next, &x_next, max, verts, alloc)) |maximal_clique| {
            if (maximal_clique.items.len > max.items.len) {
                print("New max {}\n", .{maximal_clique.items.len});
                max.* = maximal_clique;
            }
        }

        _ = p.orderedRemove(0);
        try x.append(alloc, p_vert);
    }
